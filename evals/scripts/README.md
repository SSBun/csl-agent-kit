...
- `validate`：校验 74-case v3 corpus、main-session L0–L4 decisions、task-plan → task Target 继承、delegated child handoff、S0／S1 overlays、contrast pairs 和 reason requirements。
...
- `--self-test`：验证 perfect baseline 以及 under-guard、over-guard、checkpoint、task-plan handoff 重复确认、L3／L4、safety、child confirmation 和 stale-plan regressions。

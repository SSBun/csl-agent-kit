...
4. Read the smallest sufficient entries, implementation, config, boundaries, and tests. Trace backward from the required observable result. Inside the report, use plain repository-relative anchors: prefer `path#symbol`, then `path#key`, then line numbers. README proves intent only; CodeGraph is navigation only.

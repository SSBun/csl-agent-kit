...
- `inner:refresh-tab-title` 只在模型把最新 prompt 判定为新主题时写入 OSC 标题；确认、继续、重试及 commit/push/test 等例行跟进返回 `KEEP_CURRENT_TITLE` 并保持当前标题，模型失败仍使用本地 prompt 摘要。权威实现与回归入口为 `skills/triggerify/scripts/refresh-tab-title.js` 和 `tests/triggerify.test.js`。

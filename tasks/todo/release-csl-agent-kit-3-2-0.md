...
- T5（进行中）：GitHub Actions run `30622260170` 在 `npm ci` 阶段稳定复现 `Missing: yaml@2.9.0 from lock file`；`npm install --package-lock-only` 同步 lockfile后，本地 `npm ci`、`npm run check` 及 run `30622447586` 的 Node 18/20 jobs 均通过。该 run 随后暴露 workflow 的旧顶层目录检查不支持 nested skills，而 `npm run check` 已由 `Claude plugin exports every project-owned leaf skill` 测试递归验证同一契约，因此删除重复且错误的 workflow step；等待新的最终 CI。

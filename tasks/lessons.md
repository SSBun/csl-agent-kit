...
- **Rule:** 跨会话约定放入始终在场的载体（`~/.csl-agent-kit/conventions.md`）并经 `AGENTS.md` 引用 + `SessionStart` hook 一次性注入；不要用关键词按 prompt 匹配注入。按需注入只适用于“特定上下文才触发”的内容，用它承载“应始终在场”的内容会因关键词未命中而失效。
...
- **Trigger:** 为了调整某个 skill 的触发、循环、报告字段或中间记录行为，准备修改通用或项目 `AGENTS.md`。
  - **Rule:** `AGENTS.md` 只保留跨任务稳定的高层原则；把具体触发边界、状态同步和输出契约放入对应 skill 与 eval。审查报告只持久化用户关心的讨论结果和最终决定，不把轮次、fingerprint、技术附录等运行时管理信息写入最终文件。
  - **Why:** 高频修改 `AGENTS.md` 会破坏 prompt cache、增加无效 token；把 skill 内部 ledger 每轮投影到文件也会制造没有用户价值的工具调用。

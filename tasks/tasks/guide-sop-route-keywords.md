...
- T2: Yao lint/governance/structure 通过，仅保留允许的初始加载 token 预算失败；test:tasks 与 git diff --check 通过。
- Review gate: Skipped — 用户未要求独立审查。

## Verification

- Passed: Yao 校验除允许的 token 预算外全部通过；npm run test:tasks 与 git diff --check 通过。

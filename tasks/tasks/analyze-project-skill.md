...
- [x] 添加最小 trigger/output eval 与契约检查，运行 Skill Creator、local quality gate 和仓库测试。
...
- [ ] 按项目规则复跑 local quality gate 审计与相关回归验证。
...
- 当前 `optimize-analyze-project` 任务已恢复单一系统化 project/component report，并退役本任务的独立 Learn 产品。
- 旧 R1 task-specific runner 不再实施；当前合同将声明式 cases 作为人工核对输入，并以 fresh-session 实际项目报告验证价值。
- 本任务保持 Aborted；既有双模式 review、need-driven eval 与其他历史证据均不代表当前合同。

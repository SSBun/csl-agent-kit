...
- [x] T5: Triggerify 聚焦测试、skill 校验和资源边界检查通过，或仅保留规则允许的 local quality gate token-budget 非阻塞项。
...
- [x] T9: 新数据流有聚焦测试，并重新通过 Triggerify 与 local quality gate 校验。
...
- T9: Triggerify 测试 27/27、Node 语法检查和标题 self-test 通过；local quality gate 结构、lint、governance 通过，唯一非阻塞项仍为 initial-load token budget（1372 > 1000）。

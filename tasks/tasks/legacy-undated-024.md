...
- [x] 运行聚焦与全量检查、打包检查和 `skill-quality` 审计。
...
- `skill-quality` 审计记录上游包的既有质量缺口：13 项均缺少 `agents/interface.yaml`，5 项超过 1,000-token 入口预算，所有项缺少可选 `manifest.json`。为保持上游来源可更新且不引入一次性元数据，未做本地改写。

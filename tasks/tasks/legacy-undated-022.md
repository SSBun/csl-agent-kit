...
- `skill-quality` lint、资源边界、治理与 11 条 trigger eval 通过；初始技能体为 509/1,000 token。治理仍仅警告项目既有的无 `manifest.json` 约定；`validate_skill.py` 仍仅要求未采用的 `agents/interface.yaml`，按现有避免一次性 agents 元数据目录的约定不新增。

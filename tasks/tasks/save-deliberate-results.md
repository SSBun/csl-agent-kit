...
3. 运行定向契约检查、trigger eval、local quality gate 审计和 skill 资源边界检查。
...
- T3: 新增 `persist_final_result` workflow case；定向契约断言、JSON 解析、`test:tasks` 14/14、trigger eval 31/31（precision/recall 1.0）通过。local quality gate syntax、lint、governance 通过；资源边界仅保留项目允许的初始加载预算超限（1824/1000）。

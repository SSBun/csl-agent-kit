...
- [x] 运行所需 local quality gate skill 审计并记录已知非阻塞缺口。
...
- JSON、hook parity、残留路径和 `git diff --check` 验证通过；brainstorming 与 tips quick validation 通过。local quality gate lint、governance、resource boundary 通过，聚合结果仅保留仓库统一的 `Missing agents/interface.yaml` 约定缺口。

...
- [x] T8：更新后的路由、契约、语言和 skill package 通过适用项目测试、Yao 与资源边界验证。
...
4. 运行任务索引、package、Yao、资源边界及项目验证。
...
- T4：README 与 Claude manifest 已加入 `deep-explore`；`npm run check`、Yao validation、resource-boundary check、JSON/契约 fixture 检查及 `git diff --check` 全部通过，初始加载估算为 `776/1000` tokens。
...
- T8：语义 phrase contract 为 precision `1.0`、recall `1.0`；JSON、语言、报告契约和新目录检查通过；`npm run check` 全部通过。Yao syntax/lint/governance 通过；resource-boundary 唯一未满足项是允许的初始加载预算 `1979 > 1000`，无其他资源边界失败。

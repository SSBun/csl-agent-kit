...
- T4: CLI tests passed 26/26, the two moved-rule contract tests passed, the real-HOME wrapper dry-run planned four default relinks without changing installed links, and package, syntax, JSON, tree, and `git diff --check` checks passed. Yao trigger evaluation remains 37/37 from the unchanged rule content. Full `npm test` remains red only in the two pre-existing task-suite contracts recorded earlier.

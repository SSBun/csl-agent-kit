...
- Keep the complete operational contract in the main `SKILL.md`; Yao's 1000-token initial-load budget remains advisory only.
...
- [x] T4: Representative positive, negative, and boundary cases plus contract tests enforce the new behavior, and all applicable validation passes except the accepted Yao initial-load budget result.
...
- T4: Both focused workspace-context tests passed; `quick_validate.py` passed; Yao validation passed syntax/frontmatter, lint, governance, and all other resource checks, with only the accepted `Estimated initial-load tokens exceed budget: 2353 > 1000` result.

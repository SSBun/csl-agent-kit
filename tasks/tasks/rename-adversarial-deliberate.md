...
- [x] T3: 技能路由评估与 local quality gate 校验通过；资源边界检查除允许的初始加载 token 预算超限外无失败。
...
- T3: trigger eval 31/31 通过，precision/recall 均为 1.0；local quality gate 的 syntax、lint 与 governance 检查通过。`context-budget check` 唯一失败为允许的初始加载预算 `1641 > 1000`。

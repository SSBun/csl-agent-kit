...
test("default agent instructions explain workspace records and route work to workflow skills", () => {
  ...
  assert.match(rules, /sole detailed alignment authority/);
  assert.match(rules, /current user authorization/);
  assert.match(rules, /Present every ready Target for a concrete, non-trivial outcome once before substantive work/);
  assert.match(rules, /activated solely for a trivial deterministic file edit may omit the informational display/);
  assert.match(rules, /materially equivalent .* continues without redundant confirmation/);
  assert.match(rules, /materially different Target must be presented and explicitly accepted/);
  ...
  assert.match(alignmentProtocol, /sole detailed authority for Task Target readiness, semantic alignment, mandatory non-trivial presentation, conditional confirmation, revision, and realignment/);
  ...
  assert.match(alignmentProtocol, /For every ready candidate Target that represents a concrete, non-trivial outcome, render the following plain Markdown structure/);
  assert.match(alignmentProtocol, /continue immediately without waiting for a user response/);
  assert.match(alignmentProtocol, /activated solely for a trivial deterministic file mutation and the candidate is materially equivalent/);
  assert.match(alignmentProtocol, /every non-equivalent candidate must still use the rendered block/);
  assert.match(alignmentProtocol, /end the turn merely because the Target was displayed/);
  assert.doesNotMatch(alignmentProtocol, /continue without rendering a confirmation prompt/);
  ...
  assert.match(alignmentProtocol, /trimmed content is exactly `1` or case-insensitively equals `y` confirms only a currently presented Target that is awaiting confirmation/);
...
test("injected CSL Agent Kit contract defines behavior without runtime mechanics", () => {
  ...
  assert.match(contract, /Always present every ready Task Target for a concrete, non-trivial outcome once before substantive work/);
  assert.match(contract, /activated solely for a trivial deterministic file mutation may omit the informational display/);
  assert.match(contract, /A materially equivalent Target continues without waiting for redundant confirmation/);
  assert.match(contract, /a materially different Target must be presented with a confirmation instruction and explicitly accepted/);
  assert.match(contract, /Present a complete explicit user revision and continue when the revised Target is materially equivalent/);
  assert.match(contract, /material change to the committed result or boundary/);

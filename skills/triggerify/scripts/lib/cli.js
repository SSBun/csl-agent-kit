...
function listCommand(options, io) {
  ...
  if (options.json) io.log(JSON.stringify(items, null, 2));
    ...
    io.log("ID\tDESCRIPTION\tDEFAULT\tOVERRIDE\tCONFIGURED\tVALIDATION\tTRUST\tSUPPORT\tEFFECTIVE\tEVENT\tACTION");
    items.forEach((item) => io.log(`${item.id}\t${item.description || "-"}\t${item.default}\t${item.override}\t${item.configured}\t${item.validation}\t${item.trust}\t${item.support}\t${item.effective}\t${item.event || "-"}\t${item.action || "-"}`));
  }
  return 0;
}
...
function showCommand(options, io) {
  ...
  return entry.valid && entry.innerConfigValid !== false ? 0 : 1;
}
...
function toggleCommand(command, options, io) {
  ...
  if (id.startsWith("inner:")) {
    resolveEntry(id, workspace);
    setInnerHookEnabled(id, enabled);
    io.log(`${enabled ? "Enabled" : "Disabled"} ${id}`);
    return 0;
  }
  ...
  if (matches.length > 1 && enabled) throw new Error(`Trigger ID conflict: ${id}`);
  for (const match of matches) setEntryEnabled(readEntry(match, workspace), enabled);
  io.log(`${enabled ? "Enabled" : "Disabled"} ${id}${matches.length > 1 ? ` (${matches.length} conflicting files)` : ""}`);
  return 0;
}

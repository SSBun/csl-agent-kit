...
3. Create one Auto parent with integration-level Targets:
   `create <parent-id> --title <title> --kind auto --target "T1: <integrated outcome>"`.
4. Create a child only for an independently acceptable or independently blocked outcome. Each child has its own observable Targets and complete `csl-task` lifecycle.
...
3. Resume and execute that child under the full `csl-task` workflow, including Result, review, verification, completion, and index check.

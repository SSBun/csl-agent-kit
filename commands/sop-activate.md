---
description: Activate a built-in SOP as a Claude Code rule. Cursor users should use agent-sops instead.
allowed-tools: Bash, Read, Write, AskUserQuestion, Glob
---

Activate a built-in SOP so it loads as a native Claude Code rule. **Claude Code only** — Cursor users should use the `agent-sops` skill's Activate SOP workflow; this command does not write Cursor rules. Codex has no rules directory.
...
```bash
ls "${CLAUDE_PLUGIN_ROOT}/skills/meta/agent-sops/sops/" 2>/dev/null
```
...
```bash
cat "${CLAUDE_PLUGIN_ROOT}/skills/meta/agent-sops/sops/{selected}.md"
```
...
globs: {globs array}
alwaysApply: false
source: agent-sops:{original-name}
---
```

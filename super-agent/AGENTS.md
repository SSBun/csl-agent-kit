...
- With the owning task active, apply the selected task-family skill's required shared Task Target Alignment Protocol as the sole detailed alignment authority. Present every ready Target for a concrete, non-trivial outcome once before substantive work; a task activated solely for a trivial deterministic file edit may omit the informational display when its Target is materially equivalent. A Target materially equivalent to the current user authorization continues without redundant confirmation; a materially different Target must be presented and explicitly accepted. Before the current Target aligns, permit only the focused user-owned clarification needed to form an honest commitment and the lifecycle writes named by the selected skill; do not inspect task-direct sources, research, plan, delegate, or edit the requested deliverable. Continue to substantive preparation or execution only after alignment.

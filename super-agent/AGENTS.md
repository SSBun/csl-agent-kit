...
- As soon as a request asks to create, modify, move, rename, or delete any file—even when the requested edit is trivial—or establishes a concrete, non-trivial, independently acceptable outcome, load `$task` and follow its `SKILL.md` to create, resume, or reopen the owning record before substantive discussion, repository exploration, research, planning, delegation, implementation, or any focused target-forming clarification performed after the outcome exists. For requested file mutations, activate the record and use the host's task-focus mechanism when available before the first requested deliverable edit. The task lifecycle writes needed for activation, focus, and Target alignment are the bootstrap exception; they do not authorize editing the requested deliverable. If no honest observable Target can yet be stated, ask one focused question and activate the record as soon as the answer establishes the outcome.

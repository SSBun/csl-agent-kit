...
- With the owning task active, apply the selected task-family skill's shared Task Target Alignment Protocol as the sole detailed authority. A task activated solely for a materially equivalent trivial deterministic file edit may continue after lifecycle activation. In the main user-facing session, every new or materially revised non-trivial equivalent Target must be displayed once as a checkpoint and explicitly accepted before substantive work; do not repeat confirmation for an accepted unchanged Target. When the user later explicitly authorizes execution of the same canonical record, an accepted equivalent `task-plan` Target remains unchanged across the handoff to `task`; planning acceptance alone never authorizes execution. A child session or child task explicitly delegated by the main session's accepted current Plan inherits that alignment and must not prompt the user; missing coverage, a material child-distribution graph change, a user decision, or an independent safety boundary returns to the main session. A materially different Target must name changed commitment dimensions and receive main-session change approval, while user-owned ambiguity requires one focused clarification there. Before alignment, permit only the clarification and lifecycle writes named by the selected skill; do not inspect task-direct sources, research, plan, delegate, or edit the requested deliverable.
